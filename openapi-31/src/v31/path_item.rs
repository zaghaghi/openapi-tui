...
impl PathItemOrRef {
  ...
  pub fn resolve(&self) -> Result<PathItem, PathItemErr> {
    match self {
      PathItemOrRef::PathItem(path_item) => Ok(path_item.clone()),
      PathItemOrRef::Ref { r#ref: _ } => Err(PathItemErr::Ref),
    }
  }
}
